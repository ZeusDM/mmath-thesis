...
- **p. 21 and 24:** The alternative description of $`\mathsf{coinv}(T)`$ from the alternative description of $`\mathsf{inv}(T)`$ is incorrect. It should be removed.
- **p. 24:** Proposition 2.4 and its proof contain a minus sign multiplied by $`\left|{\{(i, j) \mid i < j \text{ and } \alpha_i < \alpha_j \text{ and } \sigma_i > \sigma_j\}}\right|`$. It should contain a plus sign instead. Consequently, the description of $`\mathsf{coinv}(T)`$ contains a plus sign that should be a minus sign.
