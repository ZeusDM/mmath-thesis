...
\usepackage{tikz}
\usetikzlibrary{calc, patterns, arrows, arrows.meta, positioning, arrows.meta}
...
\part{Macdonald Polynomials}

\begin{frame}{Macdonald Polynomials}
  There are many families of Macdonald polynomials:
  \begin{itemize}
    \item Symmetric Macdonald polynomials \(P_\lambda\).
    \item Integral form symmetric Macdonald polynomials \(J_\lambda\).
    \item Modified Macdonald polynomials \(\tilde{H}_\lambda\).
    \item Nonsymmetric Macdonald polynomials \(E_\alpha\).
    \item Permuted basement Macdonald polynomials \(E_\alpha^\sigma\).
    \item Integral form permuted basement Macdonald polynomials \(\mathcal{E}_\alpha^\sigma\).
  \end{itemize}
\end{frame}
...
\begin{frame}{Leg, attacking, arm sets of \(u = (i, r)\)}
  \begin{minipage}[t]{.3\textwidth}
    ...
    \end{gather*}
    \begin{equation*}
      \leg(u) = \left|\Leg(u)\right| = \alpha_i - r
    \end{equation*}
    }
  \end{minipage}
  \hfill
  \begin{minipage}[t]{.3\textwidth}
    ...
    \begin{gather*}
      \Attack(u) = \\
      \left\{(j, r-1) : j < i\right\} \\
      \cup \left\{(j, r) : j > i\right\}
    \end{gather*}
    \begin{center}
  ...
  \begin{minipage}[t]{.3\textwidth}
    ...
    \begin{gather*}
      \Arm(u) = \\
      \left\{(j, r-1) : j < i, \alpha_j < \alpha_i\right\} \\
      \cup \left\{(j, r) : j > i, \alpha_j \leq \alpha_i\right\}
    \end{gather*}
    \begin{equation*}
      \arm(u) = \left|\Arm(u)\right|
    \end{equation*}
    }
  \end{minipage}
...
\begin{frame}{Alphabet}
  ...
  Define \(\ind{a, b} = 1\) if \(a > b\), and \(0\) otherwise.
  E.g. \(\ind{5, 3} = 1\), \(\ind{3, 5} = 0\), \(\ind{3, 3} = 0\).

  \vspace{1em}
  \pause

  \begin{minipage}{.45\textwidth}
  Define \(\ind{a, b, c} = \ind{a, b} + \ind{b, c} - \ind{a, c} \in \{0, 1\}\). E.g.
  \[
    \ind{1, 3, 2} = \ind{1, 3} + \ind{3, 2} - \ind{1, 2} = 0 + 1 - 0 = 1
  \]
  \[
    \ind{1, 2, 3} = \ind{1, 2} + \ind{2, 3} - \ind{1, 3} = 0 + 0 - 0 = 0
  \]
  \end{minipage}
  \hfill
  \begin{minipage}{.45\textwidth}
...
\begin{frame}{Statistics on Fillings: Major Index \(\maj\)}

For \(u = (i, r)\), define its \textbf{south} \(\south{u} = (i, r - 1)\), the box directly below \(u\).

\vspace{1em}

A box \(u\) is a \textbf{descent} if \(\ind{{T(u)}, {T(\south{u})}} = 1\), i.e. if \(T(u) > T(\south{u})\).

\vspace{1em}

The \textbf{major index} of \(T\) is
\[\maj(T) = \displaystyle \sum_{\text{descent } u \text{ in } T} (\leg(u) + 1)\]
...
\end{figure}
\[
  \maj(T) = \textcolor<2>{red!80!black}{1} + \textcolor<3>{red!80!black}{1} + \textcolor<4>{red!80!black}{2} = 4
\]
\end{frame}

\begin{frame}{Statistics on Fillings: Inversion and Coinversion}
  The boxes \((u, v, w)\) form a \textbf{triple} if
  \(u \in \dg(\alpha)\),
  \(v \in \Arm(u)\),
  \(w = \south{u}\).
  ...
  \begin{minipage}[c]{0.4\textwidth}
    \centering
    {\footnotesize Type A triples:}
    \begin{tikzpicture}[x=.4cm, y=.4cm,baseline={(0,0)}]
      \draw (0,0) rectangle (1,1); % u
      \draw (3,0) rectangle (4,1); % v
      \draw (0,-1) rectangle (1,0); % w
      \node at (.5,.5) {$u$};
      \node at (3.5,.5) {$v$};
      \node at (.5,-.5) {$w$};
      \node at (2,.5) {\small $\cdots$};
      \node[below=1.5pt] at (.5,-1) {\scriptsize $\alpha_i$};
      \node[below=1.5pt] at (3.5,-1) {\scriptsize $\alpha_j$};
      \node[below=.8pt] at (2,-1) {\scriptsize $\geq$};
    \end{tikzpicture}
  \end{minipage}
  \begin{minipage}[c]{0.4\textwidth}
    \centering
    {\footnotesize Type B triples:}
    \begin{tikzpicture}[x=.4cm, y=.4cm,baseline={(0,0)}]
      \draw (0,0) rectangle (1,1); % u
      \draw (-3,-1) rectangle (-2,0); % v
      \draw (0,-1) rectangle (1,0); % w
      \node at (.5,.5) {$u$};
      \node at (-2.5,-.5) {$v$};
      \node at (.5,-.5) {$w$};
      \node at (-1, -0.5) {\small $\cdots$};
      \node[below=1.5pt] at (.5,-1) {\scriptsize $\alpha_i$};
      \node[below=1.5pt] at (-2.5,-1) {\scriptsize $\alpha_j$};
      \node[below=1pt] at (-1,-1) {\scriptsize $<$};
    \end{tikzpicture}  
  \end{minipage}

  \vspace{2em}
  
  A triple \((u, v, w)\) in \(T\) is an \textbf{inversion} triple if \(\ind{{T(u)}, {T(v)}, {T(w)}} = 1\), and a \textbf{coinversion} triple otherwise.

  \colorlet{inversionbase}{orange}
  \colorlet{coinversionbase}{purple}

  \[
    \textcolor{inversionbase!60!black}{\inv(T) = \#\text{inversion triples in } T}
    \quad
    \textcolor{coinversionbase!60!black}{\coinv(T) = \#\text{coinversion triples in } T}
  \]

  \begin{figure}[htbp]
    \colorlet{coinversion}{coinversionbase!50}
    \colorlet{inversion}{inversionbase!50}
    ...
    \end{tabular}
  \end{figure}
  \[ \textcolor{inversionbase!60!black}{\inv(T) = 2} \quad \textcolor{coinversionbase!60!black}{\coinv(T) = 3}\]
\end{frame}
...
\begin{frame}{Plethysm and Superization}
  Recall that the modified Macdonald polynomial \(\tilde{H}_\lambda\) is related to the integral form Macdonald polynomial \(J_\lambda\) by a plethystic substitution:
  \[J_\lambda[X; q, t]
  = t^{\#(\text{triples in } \lambda) + |\lambda|} \tilde{H}_{\lambda'}\left[X (t^{-1} - 1); t^{-1}, q\right].\]

  \vspace{1em}

  Combinatorially,
  plethysm corresponds to \textbf{superization},
  which uses an signed alphabet with positive and negative labels.
  For us:
  \begin{equation*}
    \mathcal{A}_{\pm} = [n]_{\pm} = \{1 < \bar{1} < 2 < \bar{2} < \cdots < n < \bar{n}\},
  \end{equation*}
  \begin{equation*}
    \ind{a, b} = 1 \text{ iff } a > b \text{ or } (a = b \text{ with both negative}).
  \end{equation*}
  Everything is the same, but with this extended definition of \(\ind{a, b}\) (e.g. descents, inversions).
  \begin{gather*}
    \ind{5, 3} = \ind{5, \bar{3}} = \ind{\bar{5}, 3} = \ind{\bar{5}, \bar{3}} = 1, \\
    \ind{2, 2} = 0, \quad \ind{2, \bar{2}} = 0, \quad \ind{\bar{2}, 2} = 1, \quad \ind{\bar{2}, \bar{2}} = 1.
  \end{gather*}
\end{frame}
...
\begin{frame}{Signed Tableaux Formula for Integral Form \(J_\lambda\)}
  ...
  \qquad
  J_\lambda(X; q, t)
  = t^{\#\text{tr}(\lambda) + |\lambda|} \tilde{H}_{\lambda'}\left[X (t^{-1} - 1); t^{-1}, q\right].\]
