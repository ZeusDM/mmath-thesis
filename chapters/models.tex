...
\end{equation}
Using a superization technique,%
...
from this plethystic formula and the tableaux formula in \Cref{eq:modified-macdonald-Fill}, \cite{HHL05} derive the following combinatorial formula for \(J_\mu\) as a sum over all fillings using the signed alphabet \(\interval{n}_\pm\):
...
\begin{proof}[Proof (\cite{HHL05})]
	...
	let \(a\) be the smallest label which is the absolute value of two attacking entries in \(T\).
	Let \(v\) be the earlier box (in reading order) which is part of an attack pair with absolute value \(a\).
	...
	We have \(\mathbb{x}^{T'} = \mathbb{x}^{T}\) and \(\operatorname{neg}(T') = \operatorname{neg}(T) + 1\).

	I claim that \(\Des(T') = \Des(T)\).
	\todo{Prove this.}
	...
	\begin{align*}
		...
		&= - \mathbf{x}^{T} q^{\maj(T)} t^{\coinv(T)} (-t)^{\operatorname{neg}(T)},
	\end{align*}
	which implies that the contributions of \(T\) and \(T'\) cancel out in \Cref{eq:J-Fill-signed}.
	Hence, summing over all fillings in \(\Fill[\interval{n}_\pm](\mu)\) is equivalent to summing over only the non-attacking fillings in \(\NAF[\interval{n}_\pm](\mu)\),
	which proves that \Cref{eq:J-Fill-signed} implies \Cref{eq:J-NAF-signed}.
\end{proof}
...
with \Cref{eq:J-NAF-unsigned} for the integral form symmetric Macdonald polynomials \(J_\mu\).
Although \Cref{eq:calE-NAF-unsigned} is not obtained via a plethystic substitution, we can still use the same methods as \Cref{prop:J-NAF-signed,prop:J-NAF-unsigned} to obtain similar formulas for \(\mathcal{E}_\alpha^\sigma\).
...
\begin{proposition} \label{prop:calE-NAF-signed}
	For a composition \(\alpha\) with \(n\) parts and a permutation \(\sigma\) of \(\interval{n}\),
...
\begin{proof}
	...
	Let \(u\) be the earlier box (in reading order) which attacks \(v\) and has absolute value \(a\).
	Define \(T'\) to be the filling obtained from \(T\) by switching the sign of the entry in box \(u\).
	Then, \(T'\) is also an attacking augmented filling in \(\Fill[\interval{n}_\pm](\alpha, \sigma) \setminus \NAF[\interval{n}_\pm](\alpha, \sigma)\).
	...
	\begin{align*}
		...
		&= - \mathbf{x}^{T} q^{\maj(T)} t^{\coinv(T)} (-t)^{\operatorname{neg}(T)},
	\end{align*}
	which implies that the contributions of \(T\) and \(T'\) cancel out in \Cref{eq:calE-Fill-signed}.
	Hence, summing over all fillings in \(\Fill[\interval{n}_\pm](\alpha, \sigma)\) is equivalent to summing over only the non-attacking fillings in \(\NAF[\interval{n}_\pm](\alpha, \sigma)\),
	which proves that \Cref{eq:calE-Fill-signed} implies \Cref{eq:calE-NAF-signed}.
\end{proof}
