...
The \vocab{content} of a filling \(T\) with labels in the unsigned alphabets \(\positives\) or \(\interval{n}\) is the composition \(\beta = \content(T)\) where \(\beta_i\) is the number of boxes \(u \in \dg(\alpha)\) such that \(T(u) = i\).
The \vocab{content} of a filling \(T\) with labels in the signed alphabets \(\integers_\pm\) or \(\interval{n}_\pm\) is the composition \(\beta = \content(T)\) where \(\beta_i\) is the number of boxes \(u \in \dg(\alpha)\) such that \(|T(u)| = i\).
...
\todo[caption={Define \(\operatorname{syminv}\) and \(\operatorname{symcoinv}\).}]{Add \(\operatorname{syminv}\) and \(\operatorname{symcoinv}\), which are the \(\inv\) and \(\coinv\) statistics that one would get by interpreting an augmented filling of shape \(\alpha\) as a non-augmented filling of shape \(\alpha + (1, 1, \dots, 1)\).}

\subsection{Mistaking augmented fillings for non-augmented fillings}
...
We define new statistics \(\operatorname{syminv}\) and \(\operatorname{symcoinv}\) on augmented fillings by
\begin{equation}
	\operatorname{syminv}(T) = \inv(T')
	\quad \text{and} \quad
	\operatorname{symcoinv}(T) = \coinv(T').
\end{equation}
